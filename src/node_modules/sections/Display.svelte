<script>
  ...
  let activeLayer, canvasEl, ctx, isTarget = false
  ...
  function render() {
    $frame.render(ctx, null, $project.tool.name === 'pointer')
  ...
  const move = (x, y) => evt => {
    ...
    let multiple = evt.shiftKey ? 5 : 1

    let ids = $store.activeLayerIds

    for (let id of $store.activeLayerIds) {
      $project.currentFrame.layers[id]._x += x * PIXEL_SIZE * multiple
      $project.currentFrame.layers[id]._y += y * PIXEL_SIZE * multiple
    }
  }
