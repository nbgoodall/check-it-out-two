<script>
  ...
  function render() {
    $project.currentFrame.render({
      ctx,
      tool,
      backgroundColor: null,
      pointing: !isTarget || tool.name === 'pointer' })
  ...
  function onKeydown(evt) {
    // Erase modifier
    if (!evt.repeat && tool.eraseable && (evt.shiftKey || evt.key.toLowerCase() === 'e')) {
      $store.eraseModifier = true
    }

    return (tool.keydown && tool.keydown(evt)) || Shortcut.call(evt)
  }

  function onKeyup(evt) {
    $store.eraseModifier = false
  }
...
<svelte:window on:mousemove|stopPropagation={ onMouseMove } on:click={ outsideClick } on:keydown={ onKeydown } on:keyup={ onKeyup } />
