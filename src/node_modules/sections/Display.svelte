<script>
  ...
  function onMouseDown(evt) {
    $project.tool.mouseDown(evt, store)
  }

  function onMouseMove(evt) {
    if (evt.target === canvasEl) {
      mouse.set({ x: evt.offsetX, y: evt.offsetY })

      $project.tool.mouseMove(evt, store)
    }
  }
  ...
  function onMouseLeave(evt) {
    ...
    $project.tool.mouseLeave(evt, store)
  }

  function onMouseUp(evt) {
    $project.tool.mouseUp(evt, store)
...
<svelte:window on:mousemove|stopPropagation={ onMouseMove } on:click={ outsideClick } on:keydown={ Shortcut.call } />
