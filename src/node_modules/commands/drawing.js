...
import { AddLayers, DeleteLayers } from './layers'
...
class DrawPixels extends Command {
  static perform({ frame, pixels }) {
    ...
    // Person has selected many layers — don't want to draw on all at once.. or do we?!
    ...
    // Drawing on a new layer
      ...
      AddLayers.perform({ frameId: frame.id, layers })
    }
  }
  ...
  static perform({ frame, pixels }) {
    ...
    for (let id of frame.layerIds) {
      ...
      if (Object.keys(oldPixels).length > 0) {
        ...
        if (Object.keys(layer.pixels).length == 0) {
          // Layer is empty — delete! For combined commands...
        }
