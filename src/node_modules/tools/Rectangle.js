...
class Rectangle extends Tool {
  ...
  mouseUp(evt) {
    ...
    for (let y = y1; y <= y2; y++) {
      for (let x = x1; x <= x2; x++) {
        this.project.currentFrame.currentLayer.setPixel(x, y, this.project.color)
      }
      // pixels[y] = [
      //   ...pixels[y].slice(0, x1),
  ...
  render(ctx, layer) {
    if (this.startX && this.startY && this.x && this.y) {
      ...
      ctx.fillRect(layer.x + x1 * PIXEL_SIZE, layer.y + y1 * PIXEL_SIZE, dx * PIXEL_SIZE, dy * PIXEL_SIZE)
