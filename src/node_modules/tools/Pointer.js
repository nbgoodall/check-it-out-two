...
class Pointer extends Tool {
  ...
  mouseDown(evt) {
    ...
    if (layer) {
      ...
      if (!activeLayerIds.includes(layer.id)) {
        let multiSelect = evt.metaKey || evt.ctrlKey || evt.shiftKey

        activeLayerIds = multiSelect ? [...activeLayerIds, layer.id] : [layer.id]
  ...
  mouseUp(evt) {
    ...
    if (this.layer) {
      if (this.layersMoved) {
        ...
        const dx = Math.round((this.x2 - this.x1) / PIXEL_SIZE) * PIXEL_SIZE,
              dy = Math.round((this.y2 - this.y1) / PIXEL_SIZE) * PIXEL_SIZE
      ...
      // Clicking on layer without moving mouse and without adding a new layer (from mouse down) — remove that layer from selection!
  ...
  render(ctx) {
    ...
    if (activeLayerIds.length === 0 && Number.isInteger(this.x1) && Number.isInteger(this.x2)) {
