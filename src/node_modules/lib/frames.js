...
function pixelizeImage(pixelArr) {
  ...
  for (let x = 0; x < RESOLUTION; x++) {
    ...
    for (let y = 0; y < RESOLUTION; y++) {
      let pixel = getPixel(pixelArr, x, y)

      if (pixel) pixels[x][y] = pixel
    }
  }
...
const importImage = async src => new Promise(resolve => {
  ...
  image.onload = function() {
    ...
    let scalingFactor = Math.max(width, height) / RESOLUTION

    ctx.drawImage(image, 0, 0, width / scalingFactor, height / scalingFactor)

    let pixelData = ctx.getImageData(0, 0, RESOLUTION, RESOLUTION).data
