...
import { AddLayer, DeleteLayer } from 'commands/layers'
import { DrawPixel } from 'commands/drawing'
...
class Frame {
  ...
  addLayers({ layers }) {
    ...
    for (let layer of layers) {
      const newLayer = new Layer({ ...layer, id: layer.id || this.layerId++ })
  ...
  getPixel(x, y) {
    ...
    if (layerIds.length !== 1 || this.layerIds.length === 0) return null
  ...
  setPixel(x, y, value) {
    ...
    // Layer exists!
    if (id) {
      return DrawPixel.perform({ frameId: this.id, id, x, y, value })

    // New Layer
    } else {
      const pixels = { [x]: { [y]: value }}

      const layer = AddLayer.perform(this, { pixels })
  ...
  render(ctx, backgroundColor = null, pointing = false) {
    ...
    for (let id of [...this.layerIds].reverse()) {
      ...
      if (hoveringIds.length > 0) {
        alpha = hoveringIds.includes(id) ? 1 : 0.8
      } else {
