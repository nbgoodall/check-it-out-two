...
import { createBlankImage, render, importImage } from 'lib/frames'
...
class Layer {
  ...
  // Get unique colors
  get colors() {
    let _colors = Object.values(this.pixels).reduce((arr, row) => arr.concat(Object.values(row)), [])
  ...
  setPixel(_x, _y, value) {
    // Normalize to grid with origin 0, 0 (top left)
    let x = _x - this.x / PIXEL_SIZE,
        y = _y - this.y / PIXEL_SIZE
  ...
  merge(layer) {
    // Because of normalized pixels
    let xOffset = layer.x / PIXEL_SIZE,
        yOffset = layer.y / PIXEL_SIZE

    for (let x in layer.pixels) {
      for (let y in layer.pixels[x]) {
        this.setPixel(Number(x) + xOffset, Number(y) + yOffset, layer.pixels[x][y])
      }
    }
  }
  ...
  renderOutline(ctx, options = {}) {
    // To appear above all other things..? IDK how do this do.
  }
  ...
  static duplicate(layer) {
    ...
    let bounds = [].concat(layer.bounds)

    const _clone = Object.setPrototypeOf({ ...layer, pixels, bounds }, Layer.prototype)
