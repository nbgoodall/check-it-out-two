...
class Project {
  ...
  // position: -1 = before, 1 = after current frame
  addFrame(after = true, duplicate = false) {
    const { frames, frameIndex } = this

    let id = this.frameId++

    let newFrame = duplicate ? Frame.duplicate(frames[frameIndex], id) : new Frame({ id })

    let position = after ? 1 : 0

    this.frames = [
      ...frames.slice(0, frameIndex + position),
      newFrame,
      ...frames.slice(frameIndex + position),
    ]

    this.frameIndex += position

    return this.update()
  }

  duplicateFrame(after) {
    return this.addFrame(after, true)
  }
  ...
  async exportGif(progressCallback = () => {}) {
    ...
    for (let frame of this.frames) {
      ...
      frames.push({
        ...frame,
        image: await createCanvasImageAsync(ctx => frame.render(ctx, backgroundColor))
      })
    }
