...
import { createGif, createCanvasImageAsync } from 'lib'
...
class Project {
  ...
  async exportGif(progressCallback = () => {}) {
    let backgroundColor = this.transparent ? 'transparent' : this.backgroundColor
  ...
  save() {
    ...
    let json = JSON.stringify({ ...this, tool: this.tool.name, frames })

    return localStorage.setItem('latest', json)
  }
  ...
  static restore(key) {
    ...
    if (_object.frames) {
      _object.frames = _object.frames.map(_frame => Frame.deserialize(_frame, _object.colors, _object.backgroundColor))
    }
