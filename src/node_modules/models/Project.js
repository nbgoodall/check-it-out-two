...
class Project {
  ...
  get exportBackground() {
    return this.transparent ? '#000000' : this.backgroundColor
  }
  ...
  async exportGif(progressCallback = () => {}) {
    ...
    for (let id of this.frameIds) {
      ...
      frames.push({
        delay: frameDuration * frame.repeat,
        image: await createCanvasImageAsync(ctx => frame.render({ ctx, backgroundColor, pointing: true }))
      })
    }

    const isHex = color => color[0] === '#'

    // console.log('yo', this.exportBackground)

    // Convert colors [ '#FF00FF' ... ] to rgb array [ 255, 0, 255, ... ]
    const globalPalette = [this.exportBackground, ...this.colors].filter(isHex).join('').match(/\w{2}/g, '').map(hex => parseInt(hex, 16))
  ...
  async exportSvg() {
    ...
    return createSvg({
      ...
      height: CANVAS_SIZE,
      title: this.exportTitle,
      layers: frame.layerIds.map(id => frame.layers[id]),
      render: ctx => this.currentFrame.render({ ctx, backgroundColor: this.exportBackground, pointer: false })
    })
  }
