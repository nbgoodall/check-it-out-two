...
import { DeleteLayers, MoveLayers, MergeLayers, Cut, Copy, Paste, SortLayers } from 'commands'
...
const moveLayer = (x, y) => evt => {
  let multiplier = evt.shiftKey ? 5 : 1

  let frame = get(project).currentFrame,
   layerIds = get(store).activeLayerIds

  const dx = x * PIXEL_SIZE * multiplier,
  ...
  MoveLayers.perform({ frameId: frame.id, layerIds, dx, dy })

  return true
}

const changeLayerZIndex = delta => evt => {
  return SortLayers.perform({
    delta,
    frame: get(project).currentFrame,
    layerIds: get(store).activeLayerIds
  })
}

const selectAllLayers = evt =>
  store.update({
    tool: 'pointer',
...
const deleteActiveLayers = evt => {
  ...
  return DeleteLayers.perform({ frameId, layerIds })
}
...
const changeRepeat = amount => evt => {
  let frame = get(project).currentFrame

  frame.repeat = Math.max(0, frame.repeat + amount)
}
...
const SHORTCUTS = {
  ...
  cmd_up: changeRepeat(1),
  cmd_down: changeRepeat(-1),
...
class Shortcut {
  static call(evt) {
    // If you're focusing an input element, shortcut keys shouldn't do anything
    ...
    let shortcut = SHORTCUTS[key.toLowerCase()]
