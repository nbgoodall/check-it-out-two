...
const joinLayers = evt => {
  ...
  if (activeLayerIds.length > 1) {
    ...
    let sortedIds = activeLayerIds.sort((a, b) => frame.layerIds.indexOf(b) - frame.layerIds.indexOf(a))

    let firstLayer = frame.layers[sortedIds[0]],
        mergingIds = sortedIds.slice(1)

    for (let id of mergingIds) {
      firstLayer.merge(frame.layers[id])
    }

    frame.destroyLayers(mergingIds)

    firstLayer.updateBounds()

    store.update({ activeLayerIds: [firstLayer.id] })
  }
}
...
const _copy = (callback = null) => evt => {
  ...
  clipboard.set(JSON.stringify(activeLayers))
}
...
const paste = () => {
  ...
  let layers = JSON.parse(get(clipboard)).map(json => new Layer({ ...json })).reverse()
