...
const metadata = {
  read: function() {
    return localStorage.getObject(METADATA_KEY) || {}
  },

  update: function(value) {
    METADATA.set(value)

    return localStorage.setObject(METADATA_KEY, value)
  },
  ...
  insert: function(project) {
    ...
    _metadata[project.id] = {
      id: project.id,
      title: project.title,
      frames_count: project.frames.length,
      image: project.frames[0].image.src
    }
...
const projects = {
  create: function() {
    ...
    const project = new Project({ id, title: `Project ${ id }` })
  ...
  read: function(id) {
    let project = localStorage.getObject(storageKey(id))
  ...
  update: function(project) {
    localStorage.setObject(storageKey(project.id), project.serialize())
